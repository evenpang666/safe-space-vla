...
def _load_tick(
    ...
    *,
...
) -> tuple[dict[str, np.ndarray], dict[str, RGBDFrame], np.ndarray, np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    return rgb, scene_frames, qpos[:6], gripper, action, metadata
...
def preprocess_episode(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for tick_dir in ticks:
        rgb, scene_frames, qpos, gripper, action, metadata = _load_tick(
            tick_dir,
    ...
    payload: dict[str, Any] = {
        ...
        "gripper_geometry_mode": np.asarray("rigid_full_collision_mesh"),
